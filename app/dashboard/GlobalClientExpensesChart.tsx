...
// Component
// ---------------------------------------------------------------------------
export default function GlobalClientExpensesChart() {
  ...
  useEffect(() => {
    ...
    supabase
      .rpc("get_global_monthly_expenses_by_client", params)
      .then(({ data, error: err }) => {
  ...
  // Build chart data: [{ mes, clientA_id: n, clientB_id: n, … }, …]
  const { months, clients, chartData } = useMemo(() => {
    const monthSet = new Set<string>();
    const clientMap = new Map<string, { name: string; index: number }>();

    raw.forEach((r) => {
      monthSet.add(r.mes);
      if (!clientMap.has(r.client_id)) {
        clientMap.set(r.client_id, { name: r.client_name, index: clientMap.size });
      }
    });

    const months = [...monthSet].sort();
    const clients = [...clientMap.entries()].map(([id, { name, index }]) => ({
      id,
      name,
      colour: raw.find((r) => r.client_id === id)?.color ?? colourFor(index),
    }));
    ...
    raw.forEach((r) => {
      ...
      row[r.client_id] = (row[r.client_id] ?? 0) + r.gasto_total;
    });
  ...
  if (error)
    return (
      <p className="text-red-500 text-sm">Error cargando gastos por cliente: {error}</p>
    );
