...
function pctConsumed(t: number, budget: number | null | undefined): string {
  if (!budget) return "—";
  return `${((t / budget) * 100).toFixed(1)} %`;
}

function pctConsumedColor(t: number, budget: number | null | undefined): string {
...
// Component
// ---------------------------------------------------------------------------
export default function ProjectKpisTable({ rows }: { rows: EngagementKpi[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    const pr = projectFilter.trim().toLowerCase();
    return rows.filter((r) => {
      if (cl && !r.client_name.toLowerCase().includes(cl)) return false;
      if (pr && !r.project_name.toLowerCase().includes(pr) && !r.engagement_name.toLowerCase().includes(pr)) return false;
      return true;
    });
  }, [rows, clientFilter, projectFilter]);
  ...
  const sorted = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      ...
      switch (sortKey) {
        ...
        case "project_name":     av = a.project_name;     bv = b.project_name; break;
        case "engagement_name":  av = a.engagement_name;  bv = b.engagement_name; break;
        ...
        case "ter_val":          av = terVal(a);          bv = terVal(b); break;
        case "budget":           av = a.budget ?? -Infinity; bv = b.budget ?? -Infinity; break;
        case "remaining":        av = a.budget != null ? a.budget - terVal(a) : -Infinity; bv = b.budget != null ? b.budget - terVal(b) : -Infinity; break;
        case "pct_consumido":    av = a.budget ? terVal(a) / a.budget : Infinity; bv = b.budget ? terVal(b) / b.budget : Infinity; break;
      }
  ...
  const totalGastos = sorted.reduce((s, r) => s + (r.gasto_total ?? 0), 0);
  const totalTer    = sorted.reduce((s, r) => s + terVal(r), 0);
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          <label className="text-xs text-gray-500 dark:text-gray-400 whitespace-nowrap">Proyecto / Engagement:</label>
          <input
            type="text"
            value={projectFilter}
            onChange={(e) => setProjectFilter(e.target.value)}
            placeholder="Filtrar…"
            className="rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 px-3 py-1.5 text-sm text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 w-52"
          />
        </div>
        ...
        <span className="text-xs text-gray-400 dark:text-gray-500 self-center">
          {sorted.length} de {rows.length} engagements
        </span>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 dark:bg-gray-900 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">
              ...
              {sh("project_name", "Proyecto", false)}
              {sh("engagement_name", "Engagement", false)}
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
            {sorted.map((r) => (
              <tr key={r.engagement_id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                ...
                <td className="px-4 py-3 text-right tabular-nums font-medium text-gray-900 dark:text-gray-100">{eur.format(terVal(r))}</td>
                <BudgetCell engagementId={r.engagement_id} initialBudget={r.budget ?? null} />
                <td className={`px-4 py-3 text-right tabular-nums font-medium ${r.budget != null ? remainingColor(r.budget - terVal(r)) : "text-gray-300 dark:text-gray-600"}`}>
                  {r.budget != null ? eur.format(r.budget - terVal(r)) : "—"}
                </td>
                <td className={`px-4 py-3 text-right tabular-nums font-semibold ${pctConsumedColor(terVal(r), r.budget)}`}>
                  {pctConsumed(terVal(r), r.budget)}
                </td>
              </tr>
            ))}
          </tbody>
          <tfoot>
            <tr className="bg-gray-50 dark:bg-gray-900 border-t-2 border-gray-200 dark:border-gray-700 font-semibold text-gray-900 dark:text-gray-100">
              <td className="px-4 py-3" colSpan={4}>Total ({sorted.length})</td>
