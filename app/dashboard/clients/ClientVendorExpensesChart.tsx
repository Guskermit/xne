...
// ---------------------------------------------------------------------------
// Colour palette (same as ClientAnsrChart for visual consistency)
// ---------------------------------------------------------------------------
const COLOURS = [
...
// Component
// ---------------------------------------------------------------------------
export default function ClientVendorExpensesChart({
  clientId,
}: {
  clientId: string;
}) {
  ...
  useEffect(() => {
    ...
    supabase
      .rpc("get_client_vendor_monthly_expenses", params)
      .then(({ data, error }) => {
  ...
  const { months, vendors, chartData } = useMemo(() => {
    ...
    const vendorMap = new Map<string, string>(); // id → name
    for (const r of raw) {
      monthSet.add(r.mes);
      if (!vendorMap.has(r.vendor_id)) vendorMap.set(r.vendor_id, r.vendor_name);
    }
    const months = [...monthSet].sort();
    const vendors = [...vendorMap.entries()].map(([id, name], i) => ({
      id,
      name,
      colour: raw.find((r) => r.vendor_id === id)?.color ?? colourFor(i),
    }));

    // Lookup: mes → vendor_id → gasto
    const lookup = new Map<string, Map<string, number>>();
    for (const r of raw) {
      if (!lookup.has(r.mes)) lookup.set(r.mes, new Map());
      lookup.get(r.mes)!.set(r.vendor_id, r.gasto_total);
    }
    ...
    const chartData = months.map((mes) => {
      ...
      for (const v of vendors) {
        entry[v.id] = monthData.get(v.id) ?? 0;
      }
      return entry;
    });
  ...
  if (loading) {
    return (
      <section className="w-full max-w-7xl space-y-3">
        <h2 className="text-lg font-semibold">Gastos mensuales por proveedor</h2>
  ...
  if (error) {
    return (
      <section className="w-full max-w-7xl">
        <h2 className="text-lg font-semibold mb-2">Gastos mensuales por proveedor</h2>
